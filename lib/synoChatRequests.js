...
class SynoChatRequests {
    ...
    async sendBaseRequest(requestProperties, checkCert = null) {
        ...
        const res = await request(requestProperties);
        if (res.status === 200) {
    ...
    async initialConnectivityCheck() {
        ...
        try {
            const response = await this.sendBaseRequest(requestProperties);
            if (response.data?.success === false && response.data?.error?.code === 103) {
        ...
        try {
            ...
            if (errorCode === '411') {
                this.adapterInstance.log.warn(
                    `Attempt to send message '${msgUuid}' failed due to too fast sending sequence: '${data?.error?.errors}' > Delaying the message for ${callIteration} second/s...`,
                );
                ...
                await this.adapterInstance.delay(Math.floor(Math.random() * (1450 - 890 + 1) + 890) * callIteration);
                ...
                this.adapterInstance.log.error(
                    `Unable to send message '${msgUuid}'! Synology chat server returned 410 --> Message is too long!`,
                );
            }
        } catch (err) {
            this.adapterInstance.log.error(
