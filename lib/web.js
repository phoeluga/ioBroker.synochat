        ...
        req.on('end', async () => {
			...
			for (let i = 0; i < that.config.channels.length; i++) {
				if(synoChatOutgoingDataFieldChannelName.toLowerCase() == that.config.channels[i].channelName.toLowerCase()){
					...
					if(lookupChannelType.toLowerCase() == "outgoing"){
						lookupSuccessful = true;
						break;
					} else {
						that.adapter.log.debug(`${that.namespace} > WARN: The found channel is not an outgoing channel! > Checking next one...`);
					}
				}
			...
			if(!lookupChannelEnabled){
                that.adapter.log.debug(`${that.namespace} > Channel '${lookupChannelName}' was disabled in the adapter instance configuration! > Request will not be processed!`);
