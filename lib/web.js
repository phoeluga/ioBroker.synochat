	...
	async function restApiPost(req, res, queryParams) {
		...
		req.on("end", async () => {
			...
			for (let i = 0; i < that.config.channels.length; i++) {
				if(synoChatOutgoingDataFieldChannelName == that.config.channels[i].channelName){
					...
					if(!lookupChannelEnabled){
						...
						if(lookupChannelToken == synoChatOutgoingDataFieldToken){
							let iobrokerChannelObjectId = that.namespace + "." + synoChatOutgoingDataFieldChannelName + ".message";
							...
							try {
								...
								try {
									iobrokerChannelObject = await that.adapter.getForeignStateAsync(iobrokerChannelObjectId);
								} catch (err) {
								...
								if(iobrokerChannelObject != null && iobrokerChannelObject.val != null && iobrokerChannelObject.val == synoChatOutgoingDataFieldText){
								...
								let statusOfSet = await that.adapter.setForeignStateAsync(iobrokerChannelObjectId, synoChatOutgoingDataFieldText, true);
								if (statusOfSet) {
			...
			that.adapter.log.debug(`${that.namespace} > Unable to find an outgoing channel for the requested channel name '${synoChatOutgoingDataFieldChannelName}'! > Request will not be processed!`);
