...
function SynoChatWebHook(server, webSettings, adapter, instanceSettings, app) {
    if (!(this instanceof SynoChatWebHook)) {
        return new SynoChatWebHook(server, webSettings, adapter, instanceSettings, app);
    }
        ...
        if (pos !== -1) {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                try {
                    ...
                    _parts[1] =
                        _parts[1] === undefined ? null : decodeURIComponent(`${_parts[1]}`.replace(/\+/g, '%20'));
    ...
    async function restApiPost(req, res, _queryParams) {
        let body = '';
                            ...
                            try {
                                ...
                                try {
                                    iobrokerChannelObject =
                                        await that.adapter.getForeignStateAsync(iobrokerChannelObjectId);
                                } catch (err) {
                                    that.adapter.log.debug(
