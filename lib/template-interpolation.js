function replacePropertiesForTemplate(formattedMessage, templateString, dataSource, log) {
    ...
    while (regex.test(formattedMessage) && maxIter > 0) {
        ...
        formattedMessage = formattedMessage.replaceAll(
            String(`\${${templateString}.${replacePattern}}`),
    ...
    if (maxIter <= 0 && !!log) {
        log.error(`Infinite loop while parsing JSON detected! Returning raw text for ${templateString}!`);
...
function formatReceivedOnMessageData(obj, formatTemplate, config, log) {
    ...
    if (formatTemplate === config?.receivedNotificationManagerTemplate) {
        ...
        const readableInstances = Object.entries(instances).map(
            ([instance, _entry]) => `${instance.substring('system.adapter.'.length)}`,
        );
        formattedMessage = formattedMessage.replaceAll('${instances}', String(readableInstances.join(', ')));

        const allMessages = Object.values(instances)
            .map(byAdapter => byAdapter.messages ?? [])
            .reduce((prev, curr) => [...prev, ...curr], []);

        if (allMessages.length === 1 && !!allMessages[0].contextData) {
            formattedMessage = replacePropertiesForTemplate(
                formattedMessage,
                'contextData',
                allMessages[0].contextData,
                log,
            );
        }
    }
