...
class Synochat extends utils.Adapter {
    ...
    async onReady() {
        ...
        if (this.config.channelName || this.config.channelToken || this.config.channelType) {
            ...
            if (!this.config.webInstance) {
                ...
                let webInstanceObjects = await this.getObjectViewAsync('system', 'instance', {
        ...
        if (!this.config.synoUrl || !this.config.iobrokerHost || !this.config.webInstance) {
    ...
    async onStateChange(id, state) {
        if (state) {
            ...
            for (let i = 0; i < this.config.channels.length; i++) {
                if (channelNameFromId === this.config.channels[i].channelName.toLowerCase()) {
                    this.log.debug(
    ...
    formatObjectMessageData(obj, formatTemplate) {
        ...
        while (formatTemplate.match(/\$\{(.+?)\}/) && maxIter > 0) {
            ...
            if (wholeMatch === currentMatch) {
                ...
                let maxIterB = 100000;
                while (formatTemplate.match(`\\$\\{${currentMatch}\\.(.+?)\\}`) && maxIter > 0) {
                    ...
                    if (this.isJSON(replaceValue)) {
                        ...
                        formatTemplate = formatTemplate.replaceAll(String(`\${${replacePattern}}`), replaceValue);
                    }
    ...
    formatReceivedOnMessageData(obj, formatTemplate) {
        return TemplateInterpolation.formatReceivedOnMessageData(obj, formatTemplate, this.config, this.log);
    }
}
